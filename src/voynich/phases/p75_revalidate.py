"""
Phase 75, Track 1: Re-Validate Clean Core (3-Coda Model)
==========================================================
Re-run Phase 69 Track 0's three permutation tests under the 3-coda
decode model (connector→null AND descender→null):
  ...
  Test 0B: Coda permutation — shuffle coda stroke → consonant mappings
           (connector AND descender always null; only hook/sigmoid/vertical shuffled)
  Test 0C: Signal coherence — combined linguistic coherence test

Compare results against Phase 73 values (loaded from p73_revalidate.json).

Dependency chain:
    results/p75_redecode.json            (Step 0)
    results/p73_revalidate.json          (Phase 73, for comparison)
    results/combined_refine.json         (Phase 15)
    results/triple_tiers.json            (Phase 28/53)
    results/modifier_integrate.json      (Phase 16)
        -> results/p75_revalidate.json
"""
...
class RevalidateResult:
    phase: str = "75"
    step: str = "75.1"
    experiment: str = "revalidate_3coda"
    ...
    gate_0a: bool = False
    # Test 0B: Coda permutation (connector AND descender always null)
...
def run_revalidate_3coda() -> RevalidateResult:
    """Track 1: Re-validate clean subset with 3-coda decode (connector+descender→null)."""
    t0 = time.time()
    rd = str(_results_dir())
    N_TRIALS = 1000

    print("Phase 75.1 — Re-Validate Clean Core (3-Coda Model)")
    print("=" * 55)

    # --- Load Phase 73 results for comparison ---
    old_validation = _safe_load(os.path.join(rd, 'p73_revalidate.json'))
    old_0a_p = old_validation.get('test_0a_p', 0.0)
    old_0b_p = old_validation.get('test_0b_p', 0.0)
    old_0c_p = old_validation.get('test_0c_p', 0.0)
    ...
    corrected_coda = _build_3coda_table()  # THE KEY CHANGE: connector+descender→null
    ...
    print(f"  Clean tokens (3-coda model): {n_clean}")
    ...
    # --- Precompute blueprints (with 3-coda table) ---
    print("\n  Precomputing clean token blueprints (3-coda model)...")
    ...
    print(f"    Gate 0A: {'PASS' if gate_0a else 'FAIL'} (Phase 73 was p={old_0a_p})")

    # ===================================================================
    # Test 0B: Coda Permutation — connector AND descender ALWAYS null
    # ===================================================================
    print(f"\n  Test 0B: Coda Permutation ({N_TRIALS} trials)...")
    print("    Note: connector AND descender always null; only hook/sigmoid/vertical shuffled")

    CODA_CONSONANTS = ['l', 'm', 'n', 'r', 's', 't']
    STROKE_GROUPS = ['hook', 'sigmoid', 'vertical']  # NOT connector or descender
    ...
    for trial in range(N_TRIALS):
        ...
        # Build random coda table — connector AND descender always null
        random_coda_table = _build_3coda_table()
    ...
    print(f"    Gate 0B: {'PASS' if gate_0b else 'FAIL'} (Phase 73 was p={old_0b_p})")
    ...
    print(f"    Gate 0C: {'PASS' if gate_0c else 'FAIL'} (Phase 73 was p={old_0c_p})")
    ...
    path = _save_json(rd, 'p75_revalidate.json', asdict(result))
