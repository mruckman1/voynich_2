"""
Phase 73, Track 3: Corrected T1 Identification
================================================
Re-run Phase 68 Track 4's T1 pipeline with the corrected decode model
(connector→null). Tokens containing connectors now decode to shorter
strings, changing wildcard patterns and potentially enabling new matches.

Compare stability: how many of the 223 old T1 IDs survive the correction?

Dependency chain:
    results/p73_redecode.json          (Step 0)
    results/combined_refine.json       (Phase 15)
    results/triple_tiers.json          (Phase 28/53)
    results/p68_expanded_t1.json       (Phase 68, for stability comparison)
        -> results/p73_t1.json
"""
...
class CorrectedT1Result:
    ...
    # Gates
    gate_t1: bool = False  # >= 180 stable (80% of 223)
...
def run_corrected_t1() -> CorrectedT1Result:
    """Track 3: Re-run T1 pipeline with corrected decode."""
    t0 = time.time()
    rd = str(_results_dir())

    print("Phase 73.3 — Corrected T1 Identification")
    print("=" * 45)
    ...
    # --- Load old T1 IDs for comparison ---
    old_t1_data = _safe_load(os.path.join(rd, 'p68_expanded_t1.json'))
    old_ids = old_t1_data.get('identifications', [])
    old_n = len(old_ids)
    old_map = {i['token']: i['matched_word'] for i in old_ids if 'token' in i}
    print(f"  Old T1 identifications: {old_n}")

    # --- Step 1: Build patterns with corrected coda ---
    token_types = sorted(set(all_tokens))
    n_token_types = len(token_types)
    print(f"\n  Token types: {n_token_types}")

    print("  Building wildcard patterns (corrected coda)...")
    patterns = _build_patterns(
        token_types, full_assignment, eva_to_triple, corrected_coda,
    ...
    path = _save_json(rd, 'p73_t1.json', asdict(result))
