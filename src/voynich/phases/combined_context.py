"""
Step 35.5 – Combined Context Analysis
=======================================
Re-run PMI context analysis on the expanded signal vocabulary under
the combined spatial+10K model.

With 30%+ SIGNAL rate (vs Phase 29's 16.5%), each signal word has
roughly twice as many SIGNAL neighbours and chains should be longer.

Dependency chain:
    combined_bigrams.json      (Step 35.4 — per-token cache)
    combined_signal.json       (Step 35.3 — signal words)
    spatial_preprocess.json    (Step 35.1 — determinatives)
    signal_context.json        (Phase 29 baseline)
        → combined_context.json (this step)
"""
...
def run_combined_context() -> None:
    """Step 35.5: Combined spatial+10K context analysis."""
    t0 = time.time()

    print("=" * 70)
    print("STEP 35.5: Combined Context Analysis")
    print("=" * 70)
    ...
    print("\n  5. Confirmed-confirmed pair check ...")
    confirmed_matches = []
    for i in range(n_tokens - 1):
    ...
    print(f"     {len(confirmed_matches)} confirmed-confirmed bigram matches")
    for m in confirmed_matches[:10]:
        print(f"       {m['folio']} pos={m['pos']}: {m['w1']} {m['w2']}")

    # ── 6. Determinative-tagged context ──
    print("\n  6. Determinative-tagged context analysis ...")
    det_context: Dict[str, Counter] = defaultdict(Counter)
    for i in range(n_tokens):
        d = det_list[i] if i < len(det_list) else None
        if d and classifications[i] == 'SIGNAL':
            det_context[d][decoded[i]] += 1

    det_context_summary: List[Dict] = []
    for det_char in sorted(det_context.keys()):
        top5 = det_context[det_char].most_common(5)
        det_context_summary.append({
            'determinative': det_char,
            'n_signal_tokens': sum(det_context[det_char].values()),
            'top_words': [{'word': w, 'count': c} for w, c in top5],
        })
        print(f"     DET:{det_char}  top: "
              + ", ".join(f"{w}({c})" for w, c in top5))

    # ── 7. Load Phase 29 baseline ──
    ...
    # ── 8. Save ──
    print("\n  8. Saving combined_context.json ...")
    output = {
        ...
        'confirmed_confirmed_pairs': confirmed_matches,
        'n_confirmed_pairs': len(confirmed_matches),
        'det_context_summary': det_context_summary,
        ...
        'verdict': (f"{len(cribs)} crib candidates, {len(chains)} chains "
                    f"(longest={longest}), {len(confirmed_matches)} confirmed pairs"),
        'runtime_seconds': round(time.time() - t0, 1),
    }

    with open(os.path.join(rd, 'combined_context.json'), 'w') as f:
