"""
Phase 72, Track 1: Connector Modifier Investigation
=====================================================
Phase 59 chose connector->r based on dict-hit (23.4% vs 0.5% for l), but
Phase 71 found that connector+descender both mapping to r produces 47% of
all coda tokens as r, creating an implausible 35% passive voice rate.

This track exhaustively tests 13 possible connector values:
  6 consonants (l, m, n, r, s, t), 5 vowels (a, e, i, o, u),
  null (connector produces nothing), word boundary (connector splits token).

Also analyzes WHERE connectors appear within tokens (medial vs final).
...
Dependency chain:
    ...
    results/null_corpus.json             (Phase 17)
        -> results/phase72_connector.json
"""
...
# ---------------------------------------------------------------------------
# Build coda table with custom connector value
# ---------------------------------------------------------------------------

def _build_coda_table_with_connector(connector_value: str) -> CodaTable:
    """Build corrected coda table with a custom connector value.

    connector_value can be:
    - A consonant/vowel character: connector maps to that character
    - '' (empty): connector produces nothing (null connector)
    - ' ' (space): connector marks a word boundary within the token
    """
    table = build_coda_table_v2()  # starts with connector->r
    if connector_value == '':
        # Null: we still need 'connector' in stroke_to_coda for classification
        # but the decode will produce empty string
        table.stroke_to_coda['connector'] = ''
    elif connector_value == ' ':
        table.stroke_to_coda['connector'] = ' '
    else:
        table.stroke_to_coda['connector'] = connector_value
    return table


# ---------------------------------------------------------------------------
# Decode with custom connector
# ---------------------------------------------------------------------------

def _decode_corpus_with_connector(
    tokens: List[str],
    ...
    """Decode corpus using a custom connector value.

    If is_boundary=True, split tokens at connector positions into
    separate words (returned as space-separated strings).
    """
    decoded = []
    for token in tokens:
...
def _compute_metrics(
    ...
    if is_boundary:
        # Split decoded tokens at spaces, treat each part as a word
        all_words = []
        word_folios = []
        ...
        # Signal isolation on boundary words
        signal_stats = _run_signal_isolation(
            all_words, [['' for _ in all_words]] * 5,  # simplified null
        ...
        return {
            ...
            'bigram_z': 0.0,  # skip bigram for boundary (different token structure)
        ...
        # Bigram z (use reduced perms for speed)
...
# ---------------------------------------------------------------------------
# Cross-validation for a specific connector value
# ---------------------------------------------------------------------------

def _compute_xval_for_connector(
    decoded: List[str],
    all_tokens: List[str],
    coda_table: CodaTable,
...
# ---------------------------------------------------------------------------
# Connector position analysis
# ---------------------------------------------------------------------------

def _connector_position_analysis(
    all_tokens: List[str],
    coda_table: CodaTable,
) -> Dict[str, Any]:
    """Analyze where connector-type modifiers appear within tokens."""
    positions = {
        'total': 0,
        'n_final': 0,
        'n_medial': 0,
    ...
    for token in all_tokens:
        ...
        for pos, (role, char) in enumerate(classified):
            ...
            # Check if this is a connector-type coda
            last_stroke = coda_table.eva_modifiers.get(char)
            if last_stroke != 'connector':
                continue
    ...
    return {
        'total_connectors': positions['total'],
...
class ConnectorResult:
    phase: str = "72"
    step: str = "72.1"
    experiment: str = "connector_investigation"
    ...
    # Gates
    gate_cn1: bool = False   # Best value != 'r'
    gate_cn2: bool = False   # Best composite > r composite + 0.01
    gate_cn3: bool = False   # Best xval > r xval + 5pp
    gate_cn4: bool = False   # Null or boundary in top 3
    gates_passed: int = 0
...
def run_connector_test():
    """Track 1: Test all possible connector values."""
    t0 = time.time()
    rd = str(_results_dir())

    print("Phase 72.1 — Connector Modifier Investigation")
    print("=" * 47)
    ...
    for value_name, value in TEST_VALUES.items():
        print(f"\n  Testing connector -> '{value_name}' ({value!r})...")

        coda_table = _build_coda_table_with_connector(value)
        is_boundary = (value == ' ')

        # Decode real corpus
        decoded = _decode_corpus_with_connector(
            all_tokens, assignment, eva_to_triple, coda_table, is_boundary)

        # Decode null corpora (same connector value)
        null_decoded_list = []
        for null_tokens in null_token_lists:
            null_dec = _decode_corpus_with_connector(
        ...
        # Cross-validation (skip for boundary — different token structure)
        if not is_boundary:
            xval = _compute_xval_for_connector(decoded, all_tokens, coda_table,
                                               clean_indices)
        else:
            xval = 0.0
    ...
    for name, c in candidates.items():
        c['composite'] = (
            ...
            0.20 * min(max(c['bigram_z'], 0) / 200.0, 1.0) +
    ...
    for rank, (name, c) in enumerate(ranked, 1):
        ...
        print(f"    {rank}. {name}: composite={c['composite']:.4f} "
              ...
              f"z={c['bigram_z']:.1f}, xval={c['cross_validation']:.3f}){marker}")
    ...
    pos_analysis = _connector_position_analysis(all_tokens, base_coda_table)
    print(f"    Total connectors: {pos_analysis['total_connectors']}")
    ...
    g1 = best_name != 'r'
    g2 = best_composite > r_composite + 0.01
    g3 = best_xval > r_xval + 0.05
    g4 = 'null' in top3_names or 'boundary' in top3_names

    gates_passed = sum([g1, g2, g3, g4])

    print(f"\n  Gates:")
    print(f"    CN1 (best != r): {'PASS' if g1 else 'FAIL'}")
    print(f"    CN2 (composite > r + 0.01): {'PASS' if g2 else 'FAIL'}")
    print(f"    CN3 (xval > r + 5pp): {'PASS' if g3 else 'FAIL'}")
    print(f"    CN4 (null/boundary in top 3): {'PASS' if g4 else 'FAIL'}")
    print(f"    Total: {gates_passed}/4")

    # --- Verdict ---
    if g1 and g2 and g3:
        verdict = 'CONNECTOR_REVISED'
    elif g1 and g2:
        verdict = 'CONNECTOR_IMPROVED'
    elif g4:
        verdict = 'STRUCTURAL_CONNECTOR'
    else:
        verdict = 'CONNECTOR_R_CONFIRMED'
    ...
    path = _save_json(rd, 'phase72_connector.json', asdict(result))
    print(f"\n  Saved: {path}")
