"""
Phase 73, Step 0: Re-Decode Corpus with Connector→Null
========================================================
Phase 72 established that connector strokes have no phonetic value.
Three tracks converged independently: connector→null wins on composite
(0.612 vs 0.576), dict-hit (30.2% vs 29.0%), and cross-validation
(90.5% vs 77.9%).

This step re-decodes all 36,238 tokens under the corrected model
(connector→'') and compares against the old decode (connector→'r').
The decoded corpus is stored in the output JSON for downstream tracks.

Dependency chain:
    results/combined_refine.json         (Phase 15)
    results/modifier_integrate.json      (Phase 16)
    results/null_corpus.json             (Phase 17)
        -> results/p73_redecode.json
"""
...
class RedecodeResult:
    phase: str = "73"
    step: str = "73.0"
    experiment: str = "redecode_null_connector"
    # Corpus stats
    n_tokens: int = 0
    n_changed: int = 0
...
def run_redecode() -> RedecodeResult:
    """Re-decode the full corpus with connector→null and compare."""
    t0 = time.time()
    rd = str(_results_dir())

    print("Phase 73, Step 0: Re-Decode Corpus (connector→null)")
    print("=" * 60)
    ...
    old_coda = build_coda_table_v2()  # connector→'r'
    new_coda = _build_coda_table_with_connector('')  # connector→null

    print(f"  Corpus: {len(all_tokens)} tokens")
    print(f"  Old coda: connector→'{old_coda.stroke_to_coda['connector']}'")
    print(f"  New coda: connector→'{new_coda.stroke_to_coda['connector']}' (null)")

    # --- Decode both ways ---
    print("  Decoding with old model (connector→r)...")
    old_decoded = []
    for token in all_tokens:
        result = decode_token_cvc_v2(token, assignment, eva_to_triple, old_coda)
        old_decoded.append(result.decoded_cvc)

    print("  Decoding with new model (connector→null)...")
    new_decoded = []
    for token in all_tokens:
    ...
    print("  Generating null corpora under corrected model...")
    null_decoded_list = []
    ...
    print("  Generating null corpora under old model...")
    ...
    path = _save_json(rd, 'p73_redecode.json', asdict(result))
    print(f"\n  Verdict: {verdict}")
    print(f"  Saved: {path}")
