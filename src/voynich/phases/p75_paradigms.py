"""
Phase 75, Track 4: Corrected Paradigm Mapping (3-Coda Model)
==============================================================
Re-run paradigm mapping with the 3-coda model (connector→null,
descender→null).  Uses corrected T1 catalogue from Track 3
(p75_t1.json).

With both connector and descender mapped to null, decoded strings
are shorter still, paradigm families should be tighter, and coda-
to-case mapping uses only 3 genuine codas (n, s, t).

Dependency chain:
    results/p75_redecode.json          (Step 0)
    results/p75_t1.json                (Track 3 — 3-coda T1 catalogue)
    results/combined_refine.json       (Phase 15)
    results/p73_paradigms.json         (Phase 73, for comparison)
        -> results/p75_paradigms.json
"""
...
def _map_coda_to_case(
    ...
    With the 3-coda model only n, s, t codas remain (connector and
    descender are null).
    """
...
class CorrectedParadigmsResult:
    phase: str = "75"
    step: str = "75.4"
    experiment: str = "paradigms_3coda"
    # Paradigm stats
    n_paradigms: int = 0
...
def run_paradigms_3coda() -> CorrectedParadigmsResult:
    """Track 4: Paradigm mapping with 3-coda decode."""
    t0 = time.time()
    rd = str(_results_dir())

    print("Phase 75.4 — Corrected Paradigm Mapping (3-Coda Model)")
    print("=" * 55)

    # --- Load 3-coda decoded corpus ---
    redecode_data = _safe_load(os.path.join(rd, 'p75_redecode.json'))
    decoded_tokens = redecode_data.get('decoded_tokens', [])
    if not decoded_tokens:
        print("  ERROR: p75_redecode.json not found. Run redecode first.")
        return CorrectedParadigmsResult()

    # --- Load 3-coda T1 catalogue (Track 3) ---
    t1_data = _safe_load(os.path.join(rd, 'p75_t1.json'))
    t1_catalogue = t1_data.get('identifications', [])
    print(f"  3-Coda T1 identifications: {len(t1_catalogue)}")

    # --- Load Phase 73 paradigm data for comparison ---
    old_paradigm_data = _safe_load(os.path.join(rd, 'p73_paradigms.json'))
    old_n_paradigms = old_paradigm_data.get('n_paradigms', 0)
    old_largest = old_paradigm_data.get('largest_paradigm_size', 0)
    old_mean_size = old_paradigm_data.get('mean_paradigm_size', 0.0)
    ...
    # --- Discover paradigms from 3-coda decoded types ---
    ...
    print(f"  Paradigms found: {n_paradigms} (was {old_n_paradigms} in P73)")
    print(f"  Largest: {largest} (was {old_largest} in P73)")
    print(f"  Mean size: {mean_size:.1f} (was {old_mean_size:.1f} in P73)")
    ...
    # --- Coda-to-case mapping (3 codas: n, s, t) ---
    print("  Mapping codas to case endings (3-coda model: n, s, t)...")
    coda_case = _map_coda_to_case(
        t1_catalogue, all_tokens, decoded_tokens, coda_table, eva_to_triple)
    ...
    if not coda_case:
        print("    (no coda observations — T1 tokens may lack coda markers)")
    ...
    print(f"\n  Gates:")
    print(f"    P1 (largest ≤ 30):       {'PASS' if gate_p1 else 'FAIL'} ({largest})")
    print(f"    P2 (mean 3-10):          {'PASS' if gate_p2 else 'FAIL'} ({mean_size:.1f})")
    print(f"    P3 (≥3 consistent):      {'PASS' if gate_p3 else 'FAIL'} ({n_consistent})")
    print(f"    P4 (≥5 PREPARATION):     {'PASS' if gate_p4 else 'FAIL'} ({n_prep})")
    print(f"    P5 (≥40% identified):    {'PASS' if gate_p5 else 'FAIL'} ({100*id_fraction:.1f}%)")
    ...
    path = _save_json(rd, 'p75_paradigms.json', asdict(result))
