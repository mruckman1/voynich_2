...
│       └── word_permutation_null.py # Reviewer: word-level permutation null test (1000 trials)
├── data/
...
Phase 47 closed the internal analysis pipeline at 43.6% dict-hit. Phase 48 turns to the manuscript's **marginal annotations** — four folios (f116v, f17r, f66r, f1r) where later hands wrote readable Latin or German alongside Voynichese text — as the first external evidence source. If marginal words share meaning with adjacent Voynichese, they provide known-plaintext cribs that can constrain or correct the assignment table.

### Verdict: CRIB_SUGGESTIVE

The f116v "sera"/"cera" partial match (edit distance 1) is promising, but no propagation candidate survived the greedy acceptance filter (must not break signal words, must not degrade dict-hit by >0.5%). The assignment table is unchanged. Bigram z=59.28 (canonical). 8/8 validations pass. Gate: **PASS**.

### Track A: f116v Decode (Steps 48A.1–48A.5)

The IVTFF line `<f116v.1,@Lx> oror.sheey<!valsch vbren so nim gaf mich o>` contains two Voynichese words — "oror" and "sheey" — adjacent to an Eastern Bavarian German phrase ("valsch vbren so nim gaf mich o" — scholarly readings range from a wax-tempering recipe to a fraud accusation). T_P15 decoding: "sheey"→"sera" (10K dict hit, all Tier 1 triples), "oror"→"nene" (no dict hit, Tier 2 triples). Lattice expansion generates 24 alternative decodings for Tier 2/3 triples; 3 hit the 10K dictionary, 3 hit the 131K dictionary. Five competing scholarly readings are compiled (Albus, Petersen, Winkler, Sherwood, VoynichTemple). Best match: "sera" vs "cera" (wax) at edit distance 1 — **PARTIAL_MATCH**. Reverse engineering: 0 new triple assignments derivable (the partial match confirms existing Tier 1 triples but provides no constraint on Tier 2/3).

### Track B: Secondary Marginals (Steps 48B.1–48B.4)

f17r has no Latin-alphabet marginal text in the IVTFF transcription — only Voynichese `oteeeon.oiil`. f66r's "muß mel" (Swabian/Alsatian German for "must grind" or a flour reference) is known only from visual inspection; no machine-readable transcription exists in the IVTFF. Marginal decode via T_P15 produces 10 tokens with 131K dict hits across the two folios. Hand/dialect compilation: f116v = Hand 3 (Eastern Bavarian), f17r = uncertain, f66r = Hand 5 (Swabian/Alsatian) — likely different annotators based on IVTFF `$H=` assignments.

### Track C: Marci Annotations (Steps 48C.1–48C.4)

f1r bears annotations attributed to Johannes Marcus Marci (17th century), potentially readable under multispectral imaging. **Data status: UNAVAILABLE** — no machine-readable Marci transcription exists in any publicly accessible dataset. The comparison and corpus-test steps are implemented but produce null results pending data availability.

### Track D: Crib Propagation (Steps 48D.1–48D.5)

65 cribs collected from all tracks (Track A reverse candidates + Track B marginal decodings + cross-phase cribs from Phases 14, 28, 29). Consistency matrix computed across all crib sources. Greedy propagation: each candidate tested against signal word survival (8 bedrock words) and dict-hit delta (threshold: ≤ −0.5%). **0 propagations accepted** — all candidates either break signal words or degrade dict-hit. Canonical bigram z-score validation via `_compute_z_full()` from Phase 47: z=59.28 (exact+relaxed, 10K dict, 500 permutations), matching Phase 47 exactly. Table unchanged.

### Validation Battery

| Validation | Result |
|------------|--------|
| V1 f116v words extracted and decoded (≥2) | PASS (2) |
| V2 Competing readings compiled (≥3) | PASS (5) |
| V3 At least 1 STRONG or PARTIAL match | PASS (PARTIAL_MATCH) |
| V4 f17r/f66r marginal content extracted | PASS |
| V5 Marci data located or documented | PASS (UNAVAILABLE, documented) |
| V6 Crib consistency matrix computed | PASS |
| V7 No regression in dict-hit (≥43.1%) | PASS (43.6%) |
| V8 Bigram z maintained (≥55.0) | PASS (59.28) |

Validations: 8/8 passed. Gate: **PASS**.

### Key Findings

1. **"sheey"→"sera" is the strongest single-word crib.** All three triples in "sheey" are Tier 1 (CONFIRMED), and "sera" is a 10K Latin dictionary hit. The edit-distance-1 match to "cera" (wax) aligns with Petersen's reading of the German margin as a wax-tempering recipe. However, "sera" alone (meaning "late" or a noun form) is also valid Latin — the partial match is suggestive but not decisive.
2. **Secondary marginals are data-poor.** f17r has no Latin-alphabet text in IVTFF (only Voynichese), f66r's reading is visual-only, and Marci data is unavailable. Phase 48's external evidence is effectively limited to a single folio (f116v).
3. **The assignment table resists all propagation attempts.** 65 cribs tested, 0 accepted. The greedy filter (protect signal words + dict-hit stability) is strict by design — this confirms Phase 33's finding that T_P15 is a robust local optimum.
4. **Bigram z=59.28 is maintained.** Using the canonical Phase 47 methodology (10K dict, exact+relaxed, 500 permutations, proper 5-null-corpus SIGNAL classification), the z-score is unchanged, confirming the propagation step introduced no regression.
...
| Phase 47 | 43.6% | — | READING_ONLY (z-audit resolved, disambig not beneficial) |
| Phase 48 | 43.6% | — | CRIB_SUGGESTIVE (0 propagations, z=59.28 maintained) |
| Phase 49 | 50.4% (10K) | 1.12× | IMPROVEMENT: External LM + ED1 lattice decode (z=56 over random lattice) |

## Phase 49: Novel Computational Approaches

Phase 48 exhausted external crib evidence. Phase 49 introduces four fundamentally new computational methods — external language model scoring, spectral periodicity analysis, optimal transport alignment, and spectral graph matching — to break through the 43.6% dict-hit ceiling. The approach is inspired by ASR (automatic speech recognition) decoding: the Phase 16 assignment table plays the role of an acoustic model, the Phase 47B lattice provides alternative hypotheses per token, and an external n-gram language model trained on medieval Latin/Italian reference corpora provides the discriminative scoring that internal bigram models could not (Phase 47 proved internal scoring is circular).

### Verdict: IMPROVEMENT (7/8 validations, Gate PASS)

Track A achieves **50.4% dict-hit on a strict 10K dictionary** (up from 24.2% Phase 16 baseline) with 28 CC bigrams and 63 consecutive hits. However, detailed null analysis reveals the improvement is dominated by edit-distance-1 (ED1) expansion — 49.7% of decoded tokens are ED1-selected, and the selectivity over random lattice selection is a modest 1.12×. The result is real (z=56 over random lattice, z=284.7 over random strings) but the effect size is small once ED1 inflation is controlled for.

### Track A: External LM Lattice Decode (Steps 49A.1–49A.5)

**Step 49A.1 (LM Build)**: Char-level 5-gram and word-level 3-gram language models trained on reference corpora: Latin (73,528 tokens, vocab 17,271), Italian (11,153 tokens), Occitan (47,913 tokens), German (149,453 tokens). Combined char perplexity on held-out 10%: 11.89. Smoothing: add-0.001 (char), add-1 (word). LMs pickled to `lm_char5.pkl` and `lm_word3.pkl`.

**Step 49A.2 (Lattice + ED1)**: For each of 36,238 tokens, the Phase 47B decode lattice is expanded with edit-distance-1 variants filtered against the top 10K reference words. ED1 candidates receive a 0.3× prior penalty. Result: 24,133 tokens (66.6%) have alternatives, 18,322 (50.6%) gain ED1 expansions, mean lattice size 9.89, max 50.

**Step 49A.3 (Beam Search)**: Per-folio beam search (width 10) scores each candidate with `α·char_5gram_logprob + β·word_3gram_logprob + γ·log(prior)`. Default weights α=0.4, β=0.4, γ=0.2 yield 50.0% dict-hit (10K), 44.3% (131K), 64.7% of tokens changed from Phase 16.

**Step 49A.4 (Calibration)**: Grid search over 25 normalized (α,β,γ) configurations on a 5,000-token herbal_a subsample. Best: α=0.67, β=0.22, γ=0.11 → **56.4% dict-hit (10K)** on subsample, 7 CC bigrams. Ablation: char LM alone = 56.4% (dominant), word LM alone = 28.8%, no ED1 = 28.8%. The char-level LM is the primary driver; ED1 expansion is essential.

**Step 49A.5 (Full Decode)**: Best config applied to full corpus: **50.4% dict-hit (10K)**, 44.4% (131K), selectivity 1.12×, 28 CC bigrams, 63 consecutive hits (f116v). Per-section: herbal_a 56.0%, biological 53.7%, cosmological 51.1%, pharmaceutical 49.2%, recipes 46.3%, astronomical 38.2%.

#### Null Analysis and Caveats

The 50.4% headline requires careful decomposition:

**Source of decoded words:**
| Source | Tokens | % of corpus | Dict hits |
|--------|--------|-------------|-----------|
| Unchanged from Phase 16 | 12,754 | 35.2% | 251 |
| From original lattice alternatives | 5,481 | 15.1% | 2 |
| **From ED1 expansion** | **18,003** | **49.7%** | **18,003** |

Nearly all dict hits (98.6%) come from ED1-selected words. The system generates edit-distance-1 variants of each decoded word and picks whichever one the char LM prefers.

**Short-word inflation:** 67.1% of dict hits are 3-letter words. A random 3-letter string has a 68.2% chance of having at least one ED1 neighbor in the 10K dictionary. For length 4: 15.0%, length 5: 1.7%, length 6: 0.1%.

**Null tests:**
| Test | Description | Result |
|------|-------------|--------|
| Random lattice selection | Pick random candidate from same lattice (1000 permutations) | 45.0% (selectivity 1.12×, z=56) |
| Random strings + ED1 + LM | Same full pipeline on random chars (5 trials, len 2-6 tokens) | 44.7% vs 82.6% real (selectivity 1.85×, z=284.7) |
| Phase 16 baseline | Original decode without ED1 | 24.2% |

**Per-length selectivity (Phase 49 vs Phase 16):**
| Length | Tokens | P49 rate | P16 rate | Ratio |
|--------|--------|----------|----------|-------|
| 2 | 8,047 | 100% | 89.7% | 1.1× |
| 4 | 8,334 | 99.3% | 18.7% | 5.3× |
| 6 | 4,371 | 18.5% | 0.0% | 151× |
| 8 | 7,640 | 9.8% | 0.0% | 98× |

Genuine signal lives at lengths 4–8, where Phase 16 gets near-zero hits but the LM+ED1 pipeline recovers 10–99%. The 2–3 letter improvements are largely ED1 inflation.

**CC bigram quality:** Of 10,051 consecutive-hit pairs, only 106 (1.05%) match reference Latin bigrams. The most common pairs ("nel nel", "nel ora") are not attested Latin bigrams. The decoded vocabulary converges on a small set of generically Latin-sounding words (nel, dui, ora, seu, ben) that the char LM prefers across many tokens.

**Top decoded words:** nel (1544), dui (1354), ora (1235), seu (1102), ben (952), one (662), vada (578), nil (494), cod (493), modi (488). Only 195/4,763 unique decoded words (4.1%) are in the 10K dictionary — but these 195 words account for 50.4% of tokens by frequency.

**Fiedler vector vocabulary partition:** The Voynich word-transition graph splits cleanly into two clusters: Cluster A = long compound words (beraradu, raderadu, necoradu — likely multi-syllable tokens decoding to long strings that don't match any dictionary) and Cluster B = short functional words (ne, di, de, cone, se — the words the LM can ED1-correct into Latin). This structural split suggests the decoded vocabulary has two regimes: short tokens that are near-Latin and long tokens that remain opaque.

### Track B: Fourier/Spectral Periodicity (Steps 49B.1–49B.4)

224/226 folios show periodic structure across all 5 signal channels (frequency rank, SIGNAL indicator, gallows, dict-hit, token length), with dominant periods at 2.0–2.7 tokens. This is ubiquitous and non-discriminating — it reflects basic word-frequency alternation (common word → rare word → common word) present in any natural language text, not formulaic recipe structure. STFT detects 1,176 spectral transitions but these are distributed uniformly across sections. Spectral clustering silhouette = 0.043 — manuscript sections are NOT spectrally distinct. The periodicity finding is trivial.

### Track C: Optimal Transport Language ID (Steps 49C.1–49C.4)

PPMI+SVD embeddings (dim=50, top 500 words, window=5) are computed for the decoded Voynich vocabulary and four reference languages. Sinkhorn OT (ε=0.1): German 10.81 < Occitan 11.50 < Italian 11.50 < Latin 12.09. Gromov-Wasserstein (ε=0.1, 200-word subsample): Italian 0.0265 < German 0.0267 < Occitan 0.0298 < Latin 0.0307. Combined Borda ranking: **German #1** (score 3), Italian #2 (4), Occitan #3 (5), Latin #4 (8). The GW margin between Italian and German is negligible (ratio 1.008, discriminates_top2=False). The Sinkhorn ranking may be size-biased — German has 2× more training tokens than Latin, producing denser embeddings.

### Track D: Spectral Graph Matching (Steps 49D.1–49D.4)

Word bigram transition graphs (500 nodes) are built for each corpus. Normalized Laplacian eigenvalue spectra and NetLSD heat kernel signatures both rank **German #1**: Laplacian L2 distances — German 0.94 < Occitan 1.65 < Italian 2.91 < Latin 3.31; NetLSD — German 0.009 < Occitan 0.010 < Italian 0.030 < Latin 0.045. Tracks C and D agree on German. This is unexpected given all prior phases assumed Latin/Italian, and likely reflects that: (1) the German reference corpus is 2× larger, creating better-conditioned distance matrices, (2) the decoded Voynich vocabulary is dominated by short monosyllabic words (nel, dui, ora) whose transition structure may resemble German compounds more than Latin syntax, and (3) the Fiedler value gap (Voynich 0.630 vs German 0.521 vs Latin 0.067) suggests the Voynich transition graph is highly connected, matching German's compound-word connectivity more than Latin's sparser word-order graph.

### Track E: Integration + RL Assessment (Steps 49E.1–49E.3)

The LM reward landscape is SHARP (dynamic range 4.96, variance 5.90), making reinforcement learning feasible. Simple REINFORCE (50 episodes, softmax policy over lattice alternatives) improves dict-hit on the 500-token alternative sample by +7.0% (37.6% vs 30.6% baseline). Full RL implementation is recommended for future phases.

### Validation Battery

| Test | Description | Result |
|------|-------------|--------|
| V1 No regression: dict_hit_131k ≥ 43.6% | 44.4% | **PASS** |
| V2 Track A target: dict_hit_10k > 30% | 50.4% | **PASS** |
| V3 Track A target: CC bigrams ≥ 5 | 28 | **PASS** |
| V4 Track B target: ≥ 10 periodic folios | 224 | **PASS** |
| V5 Track C: OT discriminates top-2 languages | True | **PASS** |
| V6 Track D: Latin or Italian ranked #1 | german | **FAIL** |
| V7 Tracks C & D agree on top language | german=german | **PASS** |
| V8 LM decode perplexity < baseline | 6.37 < 11.89 | **PASS** |

Validations: 7/8 passed. Gate: **PASS**.

### Key Findings

1. **External LM + ED1 breaks the 43.6% ceiling — but most of the gain is ED1 inflation.** The char-level 5-gram LM is the dominant scorer (α=0.67); the word-level bigram LM adds little (ablation: 56.4% vs 28.8% without char LM). ED1 expansion is essential — without it, dict-hit drops to 28.8%. But ED1 means the system is not decoding the Voynich so much as finding the nearest Latin word to each decoded string. The 1.12× selectivity over random lattice selection is modest.

2. **Genuine signal exists at word lengths 4–8.** Phase 16 gets 0% dict-hit on 6+ letter tokens; the LM+ED1 pipeline recovers 18.5% (length 6) to 9.8% (length 8). At these lengths, a random string has <0.1% chance of having an ED1 neighbor in the 10K dictionary, so the LM is finding real near-matches. This is where future verification efforts should focus.

3. **Language identification unexpectedly points to German**, not Latin or Italian. Both OT and spectral graph methods agree. This likely reflects corpus size bias (German = 149K tokens vs Latin = 74K) and the monosyllabic character of the decoded vocabulary. A properly size-matched comparison is needed before drawing linguistic conclusions.

4. **The RL reward landscape is sharp.** The LM provides dynamic range 4.96 across lattice alternatives — sufficient for policy gradient optimization. This opens a path to multi-swap table optimization that escaped the flat MaxSAT landscape (Phase 44).

5. **Verification priorities for future phases:** (a) Permuted-table null — run the identical pipeline on 100 random triple→syllable tables to test whether 50% is achievable from any decode; (b) Wrong-language LM null — build German-only char LMs and compare; (c) Length-matched random word null — replace ED1 with random 10K words of matching length; (d) Bigram permutation test — permute word order within folios to test whether the 1.05% bigram match rate exceeds chance; (e) Cross-validated LM — train on 90% reference, test on 10% to prevent memorization.
...
| Phase 50 | 32.8% (10K, len≥4 ED1) | 1.10× | INSUFFICIENT_SIGNAL: ED1 artifact confirmed, Italian lang ID corrected |

## Phase 50: WFST Validation, Word-Level LM Rescoring, Extended Null Battery

Phase 49 reported 50.4% dict-hit using ED1 + char-LM lattice decoding. Phase 50 determines whether this is genuine or an ED1 artifact, then attempts the architectural fix (word-level LM + length-restricted ED1). Track A runs first; its verdict gates interpretation of everything else.

### Verdict: INSUFFICIENT_SIGNAL (3/6 validations, Gate PASS)

The Phase 49 improvement was **largely an ED1 artifact**. Any random assignment table + ED1 + char LM produces ~28% dict-hit (vs 30% real, selectivity 1.10×). The word-level Viterbi adds zero improvement. Only 1/5 null tests pass. However, two genuine findings survive: CC bigram z=21.0 confirms real sequential structure, and size-matched language ID corrects to Italian #1 (German ranking was corpus-size bias).

### Track A: Permuted-Table Null

The critical test: does the specific 25-triple assignment table matter?

50 partial permutations (shuffle 13 free triples, keep 12 confirmed locked) and 50 full permutations (shuffle all 25) are run through the identical ED1+charLM pipeline on a 5,000-token subsample.

| Condition | Dict-Hit | Selectivity | z-score |
|-----------|----------|-------------|---------|
| **Real table (T_P15)** | **30.26%** | — | — |
| Partial permutation (50 trials) | 29.56% ± 1.22% | **1.02×** | 0.58 |
| Full permutation (50 trials) | 27.50% ± 2.23% | **1.10×** | 1.24 |

The real table is only 1.10× better than fully random tables — z=1.24, not even 2σ significant. The 12 confirmed triples provide essentially zero advantage (partial selectivity 1.02×). **The ED1+charLM pipeline produces ~28-30% dict-hit regardless of what table you use.**

Verdict: **MARGINAL** (full_selectivity < 1.3).

### Track B: Word-Level LM Rescoring

Two-stage decode fixing Phase 49's failure modes: (1) restrict ED1 to words with length ≥ 4 (blocking 3-letter inflation), (2) add word-level bigram Viterbi (α=0.7) from external Latin+Italian corpus.

| Stage | Dict-Hit | Delta |
|-------|----------|-------|
| Phase 16 raw decode (no ED1) | 11.36% | — |
| Stage 1: ED1 (len≥4) + charLM | **32.78%** | +21.42% |
| Stage 2: Viterbi (α=0.7 word LM) | **32.78%** | **+0.00%** |

The word-level Viterbi adds **exactly zero** improvement over per-token ED1 selection. Though 662 words changed, these were swaps between equally-scored alternatives with no net dict-hit gain.

**Per-length breakdown (with len≥4 ED1 restriction):**

| Length | Tokens | Stage 1 rate |
|--------|--------|-------------|
| 2 | 323 | 99.4% |
| 4 | 1,217 | 98.8% |
| 6 | 1,274 | 8.9% |
...
Blocking 3-letter ED1 reduces overall dict-hit from 50.4% to 32.8%, confirming 3-letter inflation accounted for ~18 percentage points of Phase 49's headline.

**CC bigram analysis:** 32/397 consecutive-hit pairs (8.1%) match reference Latin bigrams. Bigram permutation test (1000 trials): null mean=2.04, **z=21.0**. The sequential structure in the decoded text is real and highly significant — but the word-level LM cannot exploit it for improved per-token accuracy.

**Scramble selectivity = 1.000:** Shuffling token order within folios produces identical dict-hit rates. The word LM provides no sequential benefit — it selects the same words regardless of context.

Verdict: **NO_IMPROVEMENT**.

### Track C: Extended Null Battery (5 subtests)

| Test | Result | Verdict |
|------|--------|---------|
| **C.1: Wrong-Language LM** | Latin=Italian=German=Occitan, all produce 30.22% | **INDISTINGUISHABLE** — FAIL |
| **C.2: Length-Matched Random** | Random dict words of matching length: 97.6% vs real 30.2% | **FAIL** |
| **C.3: Section-Specific** | r=−0.67 between hit rate and decoded word length | **LENGTH_CONFOUND** — FAIL |
| **C.4: Cross-Validated LM** | CV rate 30.4% vs full 30.2%, ratio 1.006 | **SIGNAL_TRANSFERS** — PASS |
| **C.5: Bigram Attribution** | 0 DIRECT CC bigrams, 3 ED1 bigrams | **FAIL** |

**C.1** is decisive: all four language-specific char LMs produce identical dict-hit rates. The char LM provides no language-specific discrimination — ED1 converges on the same short, common words regardless of LM training language.

**C.2** reveals that randomly picking a dictionary word of matching length produces 97.6% dict-hit — far higher than the actual 30.2%. The LM is actually *worse* than random dictionary lookup, meaning it actively avoids some valid dictionary words.

**C.3** shows per-section differences (herbal_a 39.7% vs astronomical 21.5%) are explained by decoded word length (r=−0.67, p=0.07): sections with shorter decoded words get more ED1 hits.

**C.4** is the lone pass: the LM generalizes to unseen test data (ratio 1.006), confirming it is not memorizing.

**C.5**: Zero CC bigrams come from the raw Phase 16 decode — all from ED1-selected words. The underlying assignment table contributes no bigram structure whatsoever.

Verdict: **NULL_BATTERY_1/5**.

### Track D: Size-Matched Language ID

Phase 49 ranked German #1 across all methods. Phase 50 re-runs with all corpora subsampled to 11,000 tokens (10 bootstrap iterations), eliminating the corpus-size bias (German had 149K tokens vs Latin 74K).
...
**Italian ranks #1, Latin #2, German drops to last.** Phase 49's German ranking was entirely a corpus-size artifact. The Italian identification is consistent with Phase 19 (Italian syllabic tachygraphy), Phase 46 Track B (nearest SBM match: Italian, distance 0.449), and the manuscript's Northern Italian provenance.

Verdict: **ROMANCE_MARGINAL** (margin 0.33 mean rank to #2).

### Validation Battery

| Test | Description | Value | Result |
|------|-------------|-------|--------|
| V1 | Track A full selectivity > 1.0 | 1.10 | **PASS** |
| V2 | Viterbi > Stage 1 (word LM helps) | +0.00 | **FAIL** |
| V3 | Null battery ≥ 3/5 | 1/5 | **FAIL** |
| V4 | Top language is Latin or Italian | Italian | **PASS** |
| V5 | Scramble selectivity > 1.05 | 1.00 | **FAIL** |
| V6 | CC bigram z > 2.0 | 21.0 | **PASS** |

Validations: 3/6 passed. Gate: **PASS** (≥3/6).

### Key Findings

1. **Phase 49's 50.4% dict-hit was an ED1 artifact.** Any random assignment table + ED1 + char LM produces ~28-30% dict-hit. The specific T_P15 table adds only ~2 percentage points (1.10× selectivity, z=1.24). The ED1 approach is not a viable decipherment path.

2. **Short-word inflation accounted for ~18 percentage points.** Restricting ED1 to words ≥4 chars drops dict-hit from 50.4% to 32.8%. Three-letter tokens were trivially matching the dictionary via ED1 neighborhoods.

3. **The word-level LM adds zero improvement.** Despite being trained on 85K Latin+Italian tokens, the word bigram model cannot discriminate between lattice candidates. Scramble selectivity = 1.000 — the Viterbi picks the same words regardless of sequential context.

4. **Language-specific char LMs are indistinguishable.** Latin, Italian, German, and Occitan char 5-gram LMs all produce identical dict-hit rates when used for ED1 selection. The char LM is not providing any language-specific signal.

5. **Two genuine findings survive:**
   - **CC bigram z=21.0** — the sequential structure in the decoded text is real and highly significant. 8.1% of consecutive dict-hit pairs match reference Latin bigrams, vs null expectation of 0.5%.
   - **Italian ranks #1 in size-matched language ID** — correcting Phase 49's German artifact. Consistent with Phase 19 (tachygraphy) and Phase 46 (SBM profiling).

6. **The fundamental bottleneck is the assignment table, not the scoring function.** The ED1 approach tries to compensate for wrong per-token decodes by finding nearby dictionary words. Phase 50 proves this compensation is language-agnostic and table-agnostic — any noise + ED1 → dict hits. Future progress requires improving the 25-triple table itself, not post-hoc word selection.
...
T1 count distribution (1,000 trials): 744 trials (74.4%) produced **zero** T1 identifications. Only 49 trials (4.9%) reached T1 ≥ 10. Nine trials (0.9%) reached or exceeded 22.

The infrastructure was NOT the bottleneck: random tables averaged 58.6 ± 10.5 signal words (comparable to T_P15's ~70) and confirmed 17.4 ± 1.2 triples (more than T_P15's 12). The bottleneck is **specificity** — T_P15's partial-decode patterns uniquely match pharmaceutical Latin words recurring across 3+ folios, while random tables produce patterns that either match nothing, match many words (failing uniqueness), or match words on too few folios.

**Word-level specificity**: 2 of T_P15's 9 distinct T1 words (radicom, stercora) were **never** produced as T1 by any of the 1,000 random tables. The remaining 7 appeared rarely: diasene (29/1000), secundi (16/1000), rabidi (8/1000), coralli (7/1000), ratione (5/1000), codex (3/1000), commune (1/1000).

**Outlier analysis**: The heaviest right-tail trials reached T1 = 44 and 45. These outliers included pharmaceutical terms but also non-pharmaceutical items and had lower CI overlap than T_P15. The p = 0.009 (9/1000 ≥ 22) accounts for this heavy right tail.

**CI overlap caveat**: Among trials with T1 > 0, CI overlap is often high because the pharmaceutical dictionary is constructed largely from reference corpus words — any word passing dictionary search is likely in CI vocabulary by construction. The **count** of identifications, not their CI membership, is what separates real from random.

**Verdict: GENUINE.** The 22 T1 word-level identifications are table-specific at p = 0.009 (z = 4.63). The pipeline's multi-layered filtering (≥ 3 folios, unique dictionary match, confidence ≥ 0.7) is highly discriminating — 74.4% of random tables produce zero T1, and only 0.9% match T_P15's count. This gives the content vocabulary permutation-tested credibility comparable to the signal words' bigram z-test (Phase 29, z = 6.14).
